...
"""
Overall this code is made for 7 servo motors, 2 servos for each shoulder, 1 for each elbow and a servo how the hand.
The hand is 3D printed and uses treads to lift the fingers into a fist.

This code is all divided into functions and different sections for easier use and testing.
"""
...
right_wheel_forward = Pin(22, Pin.OUT) # 
right_wheel_backward = Pin(3, Pin.OUT) # Connects to RW
...
""" NEIN!   - To elaborate there was a problem with the hardware wheels which caused the robot to fall over on its face, but the code is fine
def rotate_180_right(speed):
    left_wheel_forward.value(speed) # Reccomend speed 10
    right_wheel_backward.value(speed)
    time.sleep(4)
    left_wheel_forward.value(0)
...
"""
So it is over, RCJ 2026, Nitra has come to an end and so did this amazing project.
I'll be putting this on my Github for display, the robot worked as intended if we overlook some minor hardware problems regarding the wheels.
